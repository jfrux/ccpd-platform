	...
	$.widget("ui.listbuilder", {
		...
		_create: function () {
			...
			var self = this,
				...
				container = $("<div/>").addClass("uiLister")
			...
			this._list = $("<div/>").addClass("uiTokenArea")
			...
			if (!this.disabled) {
				var item0 = $("<div/>").addClass("uiTypeahead")
					.attr("tabIndex", -1)
					.insertBefore(this._list);
				...
				this._list.bind("click", function (e) {
					self._selectItem($(e.target).closest('.uiTokenArea').find('>span:last'));
				})
		...
		_add: function (token) {/*
			...
			if (!this.options.duplicates) {
				this._list.find('li>span').each(function (index, elem) {
					if ($(elem).html().match('^' + token.text + '$')) { return; }
				});
			}
			
			var self = this,
				beforeItem = this._list.find('>span:last');
			var li = $("<span/>")
				.attr("tabIndex", -1)
				.addClass("uiToken hidden");
