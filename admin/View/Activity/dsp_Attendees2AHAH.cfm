...
$(document).ready(function() {
	...
	$(".EditStatusDate").bind("click", this, function() {
		var CurrID = this.id;
		var nPerson = $.ListGetAt(this.id, 2, "-");
		var dtCurrDate = $.Trim($("#datefill-" + nPerson).html());
		...
		$("#CurrStatusDate-" + nPerson).val(dtCurrDate);
		$("#EditDateField-" + nPerson).val(dtCurrDate);
		
		$("#editdatelink-" + nPerson).hide();
		$("#datefill-" + nPerson).hide();
		$("#editdatecontainer-" + nPerson).show();
	});
	...
	$(".SaveDateEdit").bind("click", this, function() {
		var CurrID = this.id;
		var nPerson = $.ListGetAt(this.id, 2, "-");
		var nType = $("#AttendeeStatus-" + nPerson).val();
		var dtDate = $("#EditDateField-" + nPerson).val();
		
		if(nType != "" && $.Len(dtDate) > 0) {
			$.post(sRootPath + "/_com/AJAX_Activity.cfc", { method: "saveAttendeeDate", ActivityID: nActivity, PersonID: nPerson, DateValue: dtDate, Type: nType, returnFormat: "plain" },
				function(data) {
					var cleanData = $.Trim(data);
					...
					if(Status == "Success") {
						...
						$("#editdatecontainer-" + nPerson).hide();
						$("#datefill-" + nPerson).show();
						$("#editdatelink-" + nPerson).show();
					}
			});
		} else {
			addError("You must provide full date and time.",250,6000,4000);
			$("#EditDateField-" + nPerson).focus();
			$("#EditDateField-" + nPerson).val(dtStatusMask);
		}
	});
});
</script>
        ...
        <tbody>
            <cfoutput query="qAttendees" startrow="#AttendeePager.getStartRow()#" maxrows="#AttendeePager.getMaxRows()#">
                <tr id="attendeeRow-#qAttendees.attendeeId#" class="personRow AllAttendees<cfif qAttendees.personDeleted> personDeleted</cfif>" rel="##PersonOptions#PersonID#">
                    ...
                    <td><span class="MDNonMD" id="MDNonMD#qAttendees.attendeeId#"><cfif qAttendees.MDFlag EQ "Y">Yes<cfelse>No</cfif></span></td>
