<cfif CertType EQ "CME">
	<cfif qReportData.City EQ "" AND qReportData.State EQ "">
        <!--- NO LOCATION --->
		<cfset certPath = "#ExpandPath('./_reports/#Left(ReportInfo.getFileName(),Len(ReportInfo.getFileName())-4)#')#_noloc.cfr" />
    <cfelse>
		<cfset certPath = "#ExpandPath('./_reports/#ReportInfo.getFileName()#')#" />
    </cfif>
<cfelseif CertType EQ "CNE">
	<cfset certPath = "#ExpandPath('./_reports/#ReportInfo.getFileName()#')#" />
<cfelseif CertType EQ "CPE">
	Sorry but, currently we do not allow CPE certificates to be distributed online.  To obtain your CPE certificate, please contact Bill Fant.<cfabort>
<cfelse>
	<cfset certPath = "#ExpandPath('./_reports/#ReportInfo.getFileName()#')#" />
</cfif>
...
<cfreport template="#certPath#" query="qReportData" format="pdf" name="certOutput"></cfreport>

<cfcontent variable="#certOutput#" type="application/pdf">
<cfscript>
/**
