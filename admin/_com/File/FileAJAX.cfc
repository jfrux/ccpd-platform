<cfcomponent displayname="ActivityFacultyAJAX" output="false">
  <cffunction name="removeChecked" access="remote" output="true" returntype="string">
    ...
    <cfset returnVar = createObject('component','_com.returnData.buildStruct').init() />
        ...
        <cfset returnVar.setStatusMsg('Failed to remove files.') />
      ...
      <cfif (Arguments.DocList EQ "" OR Arguments.ActivityID EQ "")>
                <cfset returnVar.setStatus(false) />
                <cfset returnVar.setStatusMsg('More information is needed in order to remove these files.') />
            </cfif>
      ...
      <cfset ActivityBean = CreateObject("component","#Application.Settings.Com#Activity.Activity").init(ActivityID=Arguments.ActivityID)>
            ...
            <cfif AddCount EQ 1>
                ...
                <cfset returnVar.setStatusMsg('#AddCount# file has been removed.') />
            <cfelse>
                <cfset returnVar.setStatus(true) />
                <cfset returnVar.setStatusMsg('#AddCount# files have been removed.') />
            </cfif>
