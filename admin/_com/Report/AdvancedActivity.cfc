<cfcomponent>
	...
	<cffunction name="Run" access="remote" output="yes">
        ...
        <cfset ReportFileName = Replace(ReportType, " ", "") & "-" & Replace(ReportData.ReportDetail, " ", "") & "_#DateFormat(Now(),'MMDDYY') & TimeFormat(Now(),'hhmmss')#_#session.personId#.xlsx">
		
		<cfif Arguments.ReportType EQ "Grouping" AND ReportTypeDetail EQ 2>
        	<cfset TitleColSpan = 6>
        <cfelse>
        	<cfset TitleColSpan = 5>
        </cfif>
		...
		<poi:document name="Request.ExcelData" file="#ReportPath##ReportFileName#" type="XSSF">
			...
			<poi:sheets>                    
				<poi:sheet name="#Arguments.ReportType#" orientation="landscape">
					...
					<poi:row class="title">
						<poi:cell value="#Arguments.ReportType# (#ReportData.ReportDetail#) // #DateFormat(Arguments.StartDate,'mm/dd/yyyy')# -to- #DateFormat(Arguments.EndDate,'mm/dd/yyyy')#" colspan="#TitleColSpan#" />
					</poi:row>
					...
					<poi:row class="data">
						...
						<cfif Arguments.ReportType EQ "Grouping" AND ReportTypeDetail EQ 2>
							<cfif ReportData.ParentActivityID EQ "">
                                <poi:cell value="Y" />
                            <cfelse>
                                <poi:cell value="" />
                            </cfif>
