        ...
        <cfif Status EQ "">
				...
				WHERE 
					...
					(SELECT COUNT(ACDC.AttendeeID) AS AttCount
					 ...
					 WHERE 	(ACDC.SentFlag = 'N') AND 
                     		(A.ActivityID = act.ActivityID) AND 
                     		(A.DeletedFlag = 'N') AND
                     		(A.CompleteDate <= @EndDate)) > 0 AND
                            (act.deletedFlag ='N')
				ORDER BY act.StartDate
			</cfquery>
            
			<!--- Define NewQuery Variable and RowCount Variable --->
           	<cfset qReport = QueryNew("ActivityType,CrsTitle,CrsDate,EndDate,CourseID,OtherCourse,DidHrs,PracHrs,Students,City,State,ClinSit2,CollPTC,Cspecify,CollOth,CoSpecify")>
            <cfset RowCount = 0>
            <cfset TotalDidHrs = 0>
            ...
            <cfloop from="1" to="#qGetCDCCourses.RecordCount#" index="Index1">
                ...
                <cfscript>
					...
					querySetCell(qReport,"ActivityType",qGetCDCCourses.ActivityTypeID[Index1],RowCount);
					querySetCell(qReport,"CrsTitle",qGetCDCCourses.Title[Index1],RowCount);
                    querySetCell(qReport,"CrsDate",CDCCourseDate,RowCount);
                    querySetCell(qReport,"EndDate",Arguments.EndDate,RowCount);
            ...
            <cfloop query="qReport">
            	...
            	<cfset qReport.EndDate = DateFormat(qReport.EndDate, "MM/DD/YYYY")>
            </cfloop>
            ...
            <!--- Excel Section --->
                ...
                <cfif Arguments.MarkSent EQ "Y">
					<cfset ReportExtendedPath = ReportPath & "\" & Arguments.ReportID & "\CDC_Activity_Report_#DateFormat(Arguments.StartDate,'MDDYY')#-#DateFormat(Arguments.EndDate,'MDDYY')#_#DateFormat(Now(),'MMDDYY')##TimeFormat(Now(),'hhmmss')#_MarkedSent.xlsx">
                <cfelse>
                    <cfset ReportExtendedPath = ReportPath & "\" & Arguments.ReportID & "\CDC_Activity_Report_#DateFormat(Arguments.StartDate,'MDDYY')#-#DateFormat(Arguments.EndDate,'MDDYY')#_#DateFormat(Now(),'MMDDYY')##TimeFormat(Now(),'hhmmss')#.xlsx">
                </cfif>
                        ...
                        <poi:sheet name="CDC Activity Report" orientation="landscape">
                            ...
                            <cfloop query="qReport">
                                <poi:row style="border:2px solid BLACK;">
                                    ...
                                    <cfif qReport.ActivityType EQ 2>
                                    	<poi:cell value="#qReport.EndDate#" />
                                    <cfelse>
	...
	<cffunction name="CDCStudentReport" access="remote" output="true" returntype="any">
        ...
        <cfif Status EQ "">
			...
			<cfloop query="qGetAttendees">
                ...
                <!--- SET CDCCOURSEDATE --->
                <cfif qGetAttendees.ActivityTypeID EQ 2>
                	<cfset CDCCourseDate = Arguments.EndDate>
                <cfelse>
        ...
        <cfif Status EQ "">
            ...
            <cfif ListFind(Arguments.SectionList, "ParticipantsState", ",")>
                ...
                <cfquery name="qGetStateAttendeeInfo" datasource="#Application.Settings.DSN#">
                    ...
                    AS (
                        ...
                        WHERE   AC.CategoryID = @Container AND 
                                ...
                                a.DeletedFlag = 'N' AND 
                                act.StatusID IN (1,2,3) AND 
                                act.Sponsor = 'Cincinnati STD/HIV Prevention Training Center'
                    )
                    ...
                    SELECT	s.StateID,
                            ...
                            (SELECT	COUNT(att.PersonID)
                             ...
                             WHERE (a.AddressID = P.PrimaryAddressID) AND (a.State = s.Code)) AS AttendeeCount
                    FROM ce_Sys_State s
                    ORDER BY s.Name
                </cfquery>
                ...
                <cfquery name="qGetNonStateAttendeeInfo" datasource="#Application.Settings.DSN#">
                    ...
                    INNER JOIN ce_Person_Address a ON a.PersonID=att.PersonID
                    INNER JOIN ce_Person P ON P.PersonID = a.PersonID
                    WHERE (a.AddressID = P.PrimaryAddressID) AND (a.State IS NULL) 
                </cfquery>
            </cfif>
            ...
            <cfif ListFind(Arguments.SectionList, "Cities", ",")>
                ...
                <cfquery name="qGetCitiesByStates" datasource="#Application.Settings.DSN#">
                    ...
                    FROM DistinctAttendees att
                    INNER JOIN ce_Person p ON p.PersonID = att.PersonID
                    INNER JOIN ce_Person_Address a ON a.AddressID = p.PrimaryAddressID
                    LEFT JOIN ce_Sys_State s ON s.Code = a.State
            ...
            <!--- Excel Section --->
                ...
                <!--- Define variables used in the CreateExcel object --->
                <cfset ReportExtendedPath = ReportPath & "\" & Arguments.ReportID & "\CDC_Overview_#DateFormat(Arguments.StartDate,'MDDYY')#-#DateFormat(Arguments.EndDate,'MDDYY')#_#DateFormat(Now(),'MMDDYY')##TimeFormat(Now(),'hhmmss')#.xlsx">
