<cfcomponent displayname="ACCME Summary Report" output="no">
	...
	<cffunction name="Run" access="remote" output="yes">
		...
		<cfargument name="ReportYear" required="false" type="string" default="#year(now())#">
		...
		<cfif Status EQ "">
			<cfquery name="qACCMEData" datasource="#Application.Settings.DSN#">
				...
				WITH CTE_Activities ( ActivityID,ParentActivityID,ActivityTitle,ActivityTypeID,ActivityTypeName,ActivityLocation,City,State,Country,Sponsorship,Sponsor,CMEHrs,StatMD,StatNonMD,ReleaseDate,ActivityDate,EndDate,StatSupporters,SupportReceived,StatSuppAmount,TotalExhibit,TotalRegistration)
				AS (
					SELECT 
						A.ActivityID,
						A.ParentActivityID,
						...
						isNull((CASE A.SessionType
							WHEN 'M' THEN 
								...
								FROM         ce_Activity_FinSupport AS FS INNER JOIN
													  ce_Activity AS A5 ON FS.ActivityID = A5.ActivityID
								WHERE     (A5.DeletedFlag='N') AND (FS.SupportTypeID = 2) AND (FS.DeletedFlag = 'N') AND
									(A5.ParentActivityID = A.ActivityID) AND (A5.StatusID IN (1,2,3)) AND (A5.StartDate BETWEEN @StartDate AND @EndDate)
									OR
									(A5.DeletedFlag='N') AND (FS.SupportTypeID = 2) AND (FS.DeletedFlag = 'N') AND
									(A5.ParentActivityID = A.ActivityID) AND (A5.StatusID IN (1,2,3)) AND (A5.EndDate BETWEEN @StartDate AND @EndDate)
								)
							WHEN 'S' THEN
								(SELECT     SUM(Amount) AS Expr1
								FROM         ce_Activity_FinSupport
								WHERE     (SupportTypeID = 2) AND (DeletedFlag = 'N') AND (ActivityID=a.ActivityID))
						END),0),
						TotalRegistration = 
						isNull((CASE A.SessionType
							WHEN 'M' THEN 
								...
								FROM         ce_Activity_FinSupport AS FS INNER JOIN
													  ce_Activity AS A5 ON FS.ActivityID = A5.ActivityID
								WHERE     (A5.DeletedFlag='N') AND (FS.SupportTypeID = 3) AND (FS.DeletedFlag = 'N') AND
									(A5.ParentActivityID = A.ActivityID) AND (A5.StatusID IN (1,2,3)) AND (A5.StartDate BETWEEN @StartDate AND @EndDate)
									OR
									(A5.DeletedFlag='N') AND (FS.SupportTypeID = 3) AND (FS.DeletedFlag = 'N') AND
									(A5.ParentActivityID = A.ActivityID) AND (A5.StatusID IN (1,2,3)) AND (A5.EndDate BETWEEN @StartDate AND @EndDate)
								)
							WHEN 'S' THEN
								(SELECT     SUM(Amount) AS Expr1
								FROM         ce_Activity_FinSupport
								WHERE     (SupportTypeID = 3) AND (DeletedFlag = 'N') AND (ActivityID=a.ActivityID))
						END),0)
			...
			<cfquery name="qMaxSupport" dbtype="query">
				SELECT ifNull(MAX(statSupporters),0) As MaxSupporters
				FROM qACCMEData
			</cfquery>
