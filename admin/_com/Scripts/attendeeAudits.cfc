<cfcomponent displayname="Attendee Audits">
	<cffunction name="assessCounts" access="remote" output="yes">
		<cfquery name="qData" datasource="#application.settings.dsn#">
			WITH CTE_Stats AS (
				SELECT
					...
					ActivityEvals = (
									...
									FROM ceschema.ce_Activity_PubComponent
									WHERE     
			...
			INNER JOIN 
				ceschema.ce_Activity As Act ON Act.activityId = AttStat.ActivityID
			INNER JOIN 
				ceschema.ce_Person As Per ON Per.personId = AttStat.personId 
			INNER JOIN 
				ceschema.ce_Attendee As Att ON Att.attendeeId = AttStat.attendeeId 
			WHERE 
				TotalMissing = 0
		</cfquery>
