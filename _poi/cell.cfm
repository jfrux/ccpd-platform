...
<cfswitch expression="#THISTAG.ExecutionMode#">
	...
	<cfcase value="End">
		...
		<cfif structKeyexists(ATTRIBUTES,'href') AND len(trim(ATTRIBUTES.href))>
			<cfset link = VARIABLES.DocumentTag.createHelper.createHyperlink(1) />
			<cfset link.setAddress(ATTRIBUTES.href) />
			<cfset VARIABLES.Cell.setHyperlink(link) />
		</cfif> 
		...
		<cfif NOT StructKeyExists( VARIABLES, "CellStyle" )>
			...
			<cfset VARIABLES.CellStyle = VARIABLES.DocumentTag.CSSRule.ApplyToCellStyle(
				...
				<cfdump var="#VARIABLES.DocumentTag.DataFormat#"><cfabort>--->
			<!--- XSSF or HSSF --->
			<!--- Check to see which type of formatting we need to apply. --->
					<cfswitch expression="#ATTRIBUTES.Type#">
						...
						<cfcase value="percent">
							<!--- Use the user-defined format. --->
							<cfset VARIABLES.CellStyle.SetDataFormat(VARIABLES.DocumentTag.DataFormat.getFormat(JavaCast("string",ATTRIBUTES.PercentFormat))) />
									
						</cfcase>
					</cfswitch>
			<!---<cfswitch expression="#VARIABLES.DocumentTag.Type#">
				<cfcase value="HSSF">
					<!--- Check to see which type of formatting we need to apply. --->
					<cfswitch expression="#ATTRIBUTES.Type#">
						<cfcase value="date">
		...
		<cfif ATTRIBUTES.Wrap>
			<cfset VARIABLES.CellStyle.setWrapText(true) />
			<cfset VARIABLES.RowTag.Row.setHeightInPoints((3*VARIABLES.SheetTag.Sheet.getDefaultRowHeightInPoints())) />
			<!---
		...
		<cfif (ATTRIBUTES.ColSpan GT 1)>
			<cfset VARIABLES.SheetTag.Sheet.addMergedRegion(CreateObject( "java", "org.apache.poi.ss.util.CellRangeAddress" ).Init(
