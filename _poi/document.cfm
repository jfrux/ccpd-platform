...
<cfswitch expression="#THISTAG.ExecutionMode#">
	...
	<cfcase value="Start">
		...
		<!--- 
			NEW! 12/10/2010 (tested with POI 3.7) added by Joshua F. Rountree
			This is the type of Workbook to create, HSSF (older style) or XSSF (newer style which uses Excel 2007 OOXML)
			Defaults to HSSF for backword-compatibility
		--->
		<cfparam
		...
		<!--- Rip off file extension --->
		<cfset ATTRIBUTES.File = reReplace(ATTRIBUTES.File,"\.[^.]*$","") />
		
		<cfset VARIABLES.Type = ATTRIBUTES.Type />
	
		<!--- Determine which kind of workbook we are creating... --->
		
		<cfswitch expression="#attributes.type#">
			<cfcase value="HSSF">
				<!--- attach file extension --->
				<cfset ATTRIBUTES.File = ATTRIBUTES.File & ".xls" />
				...
				<cfif Len( ATTRIBUTES.Template )>
					...
					VARIABLES.WorkBookFactory = CreateObject("java", "org.apache.poi.ss.usermodel.WorkbookFactory").Init();
					VARIABLES.WorkBook = VARIABLES.WorkBookFactory.create(VARIABLES.FileInputStream);
					</cfscript>		
				<cfelse>
			...
			<cfcase value="XSSF">
				<!--- attach file extension --->
				<cfset ATTRIBUTES.File = ATTRIBUTES.File & ".xlsx" />
				...
				<cfif Len( ATTRIBUTES.Template )>
					...
					<cfset VARIABLES.createHelper = VARIABLES.WorkBook.getCreationHelper() />
				<cfelse>
					<!--- Create a new workbook. --->
		...
		<cfset VARIABLES.createHelper = VARIABLES.WorkBook.getCreationHelper() />
		<cfset VARIABLES.DataFormat = VARIABLES.createHelper.createDataFormat() />
