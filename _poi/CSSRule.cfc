...
<cfcomponent
	...
	<cffunction
		...
		<!--- 
		###### HSSF CUSTOM COLORS #######
		Currently Not Supported... this is some ToDo code blocks.
		HSSF types require an existing color pallete to replace the custom colors to it.
		So we're going to set the defaults as LIME Foreground with RED text like the example at:
		http://poi.apache.org/spreadsheet/quick-guide.html#CustomColors
		
		<cfif VARIABLES.DocumentTag.Type EQ "HSSF">
			<cfset ARGUMENTS.CellStyle.setFillForegroundColor(CreateObject( 
					"java",
					"org.apache.poi.hssf.util.HSSFColorLIME"
					).GetIndex()) />
			<cfset ARGUMENTS.CellStyle.setFillPattern(ARGUMENTS.CellStyle.SOLID_FOREGROUND) />
			<cfset LOCAL.Font.setColor(CreateObject( 
					"java",
					"org.apache.poi.hssf.util.HSSFColorRED"
					).GetIndex()) />
		</cfif>
		
		<cfset LOCAL.Pallete = ARGUMENTS.WorkBook.getCustomPallete() />
		
		<!--- Replacing Red with Supplied Color --->
		<cfset LOCAL.Pallete.setColorAtIndex(CreateObject( 
					"java",
					"org.apache.poi.hssf.util.HSSFColorRED"
					).GetIndex(),
		...
		<!--- 
			...
			[UPDATED 12/10/2010 by Joshua Rountree] NOW SUPPORTS CUSTOM COLORS WITH XSSF TYPE EXCEL FILES
		--->
		<cfif (
		...
		<cfelseif isHexColor(LOCAL.PropertyMap["background-color"])>
			<!--- The user has entered a "custom color" --->
			<cfset LOCAL.NewColor = CreateObject("java","java.awt.Color").decode(LOCAL.PropertyMap["background-color"]) />
			<cfset LOCAL.XSSFColor = CreateObject("java","org.apache.poi.xssf.usermodel.XSSFColor").init(LOCAL.NewColor) />
			<cfset ARGUMENTS.CellStyle.setFillForegroundColor(LOCAL.XSSFColor)>
			<!--- Set a solid background fill. --->
			<cfset ARGUMENTS.CellStyle.SetFillPattern( ARGUMENTS.CellStyle.SOLID_FOREGROUND ) />
		<cfelse>
			<!--- could be an option for an RGB Struct? rgb(255,255,255)or just { 255, 255, 255 } --->
		</cfif>
		...
		<cfloop
			...
			<cfif Len( LOCAL.PropertyMap[ "border-#LOCAL.BorderSide#-width" ] )>
				...
				<cfelseif isHexColor(LOCAL.PropertyMap[ "border-#LOCAL.BorderSide#-color" ])>
					<!--- The user has entered a "custom color" --->
					<cfset LOCAL.NewColor = CreateObject("java","java.awt.Color").decode(LOCAL.PropertyMap[ 'border-#LOCAL.BorderSide#-color' ]) />
					<cfset LOCAL.XSSFColor = CreateObject("java","org.apache.poi.xssf.usermodel.XSSFColor").init(LOCAL.NewColor) />
					<cfset LOCAL.BorderColor = LOCAL.XSSFColor>
				<cfelse>
				...
				<cfswitch expression="#LOCAL.BorderSide#">
					...
					<cfcase value="bottom">
						<!--- Set border style. --->
						<cfset ARGUMENTS.CellStyle.SetBorderBottom( JavaCast('short',LOCAL.BorderStyle) ) />
		...
		<!--- 
			...
			[UPDATED 12/10/2010 by Joshua Rountree] NOW SUPPORTS RGB / HEX CUSTOM COLORS
		--->
		<cfif (
		...
		<cfelseif isHexColor(LOCAL.PropertyMap["color"])>
					<!--- The user has entered a "custom color" --->
					<cfset LOCAL.NewColor = CreateObject("java","java.awt.Color").decode(LOCAL.PropertyMap["color"]) />
					<cfset LOCAL.XSSFColor = CreateObject("java","org.apache.poi.xssf.usermodel.XSSFColor").init(LOCAL.NewColor) />
					<cfset LOCAL.Font.SetColor(LOCAL.XSSFColor)>
		<cfelse>
