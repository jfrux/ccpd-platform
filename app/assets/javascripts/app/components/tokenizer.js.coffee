...
(($) ->
  ...
  $.uiTokenizer = (input, settings) ->
    ...
    makeDragHidden = (ui) ->
      ...
      ui.placeholder
          ...
          .css(
            height: 16
            width: 72
          ).addClass("hidden hiddenPlaceholder")
          .find(".tokenImg")
          .hide()
    ...
    is_printable_character = (keycode) ->
      if  (keycode >= 48 and keycode <= 90) or 
          (keycode >= 96 and keycode <= 111) or 
          (keycode >= 186 and keycode <= 192) or 
          (keycode >= 219 and keycode <= 222)
        true
      else
        false
