...
<cfif Attributes.Submitted NEQ "">
	...
	<cfif NOT AlreadyExists>
		...
		<cfset NewPersonInfo = Application.Com.PersonDAO.Create(PersonInfo)>
		
		<!--- END-USER EMAIL --->
		<cfmail to="#Attributes.Email#" from="do-not-reply@uc.edu" replyto="do-not-reply@uc.edu" subject="Registration Confirmation - CCPD" type="html">
		...
		<cfcookie name="Settings.LastPage" value="Member.Home?firsttime=true" />
		<cflocation url="#myself#Main.doLogin?username=#NewPersonInfo.Username#&password=#NewPersonInfo.Password#&Submitted=1" addtoken="no" />
	<!--- EXISTING MEMBER --->
	<cfelse>
		<cflocation url="/signup?Step=AlreadyExist" addtoken="no" />
		<!--- LOOKUP CS_USER INFO --->
