...
<script>
	...
	function updateRegistrants(nPage, nStatus) {
		...
		$.get(sMyself + "Activity.Attendees2AHAH", { ActivityID: nActivity, status: nStatus, page: nPage }, 
			function(data) {
	...
	$(document).ready(function() {
		...
		$("#btnStatusSubmit").bind("click",function() {
			...
			$.post(sRootPath + "/_com/AJAX_Activity.cfc", { method: "updateAttendeeStatuses", PersonList: result, ActivityID: nActivity, StatusID:$("#StatusID").val(), returnFormat: "plain" },
			function(returnData) {
				cleanData = $.trim(returnData);
				Status = $.ListGetAt(cleanData,1,"|");
				StatusMsg = $.ListGetAt(cleanData,2,"|");
		...
		$("#RemoveChecked").bind("click",function() {
			if(confirm("Are you sure you want to remove the checked attendees from this activity?")) {
				...
				$.getJSON(sRootPath + "/_com/AJAX_Activity.cfc", { method: "removeCheckedAttendees", PersonList: SelectedMembers,AttendeeList: SelectedAttendees, ActivityID: nActivity, returnFormat: "plain" },
				function(data) {
					if(data.STATUS) {
		...
		$("#RemoveAll").bind("click",function() {
			if(confirm("WARNING!\nYou are about to remove ALL attendees from this Activity!\nAre you sure you wish to continue?")) {
				...
				$.getJSON(sRootPath + "/_com/AJAX_Activity.cfc", { method: "removeAllAttendees", ActivityID: nActivity, returnFormat: "plain" },
					function(data) {
						if(data.STATUS) {
							addMessage(data.STATUSMSG,250,6000,4000);
							updateActions();
		...
		$("#PrintCMECert").bind("click", this, function() {	//CME CERTS
			if($("#PrintSelected").attr("checked") == true) {				
		...
		$("#PrintCNECert").bind("click", this, function() {	//CNE CERTS
			if($("#PrintSelected").attr("checked") == true) {						//CHECK IF PRINTSELECTED IS CHECKED
