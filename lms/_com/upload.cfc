<cfcomponent displayname="Upload" output="no">
		...
		<cfset var statusMsg = "|||fail">
			<cfswitch expression="#Arguments.Mode#">
				<cfcase value="image">
					...
					<cffile action="upload" filefield="#arguments.filefield#" destination="#FileBase#_original\" nameConflict = "MakeUnique" result="fileResult">
			...
			<cfset returnVar = {
				fileName = lcase(fileResult.serverfile),
				contentType = lcase(fileResult.contenttype),
				fileExt = lcase(fileResult.serverfileext),
				statusMsg = statusMsg
			} /><!---
	...
	<cffunction name="getThumbnail" returnType="any" access="remote" output="false">
		...
		<!--- read it in --->
		<cfset myimage = imageNew(expandPath('/_uploads/support/#prefix#_page_1.jpg'))>
	
		<!--- clean it up --->
		<cffile action="delete" file="#expandPath('/_uploads/support/#prefix#_page_1.jpg')#">
		<cfreturn imageGetBlob(myimage)>
	</cffunction>
	
	<cffunction name="imageHandler" access="remote" output="no" returnformat="plain">
		...
		<cfargument name="isBase64" type="boolean" required="no" default="false" />
		...
		<cfset imageBean.setdata_key(arguments.key)>
		...
		<!--- RESIZE --->
		<cfset imguuid = replace(createUUID(),"-","_","all") />
		
		<cfif arguments.isBase64>
			<cffile action="read" file="#expandPath('\_uploads\images\_original\#arguments.filename#')#" variable="base64Image">
			
			<cfset oImage = imageReadBase64(base64Image) />
		<cfelse>
			<cfset oImage = imageRead("#expandPath('\_uploads\images\_original\#arguments.filename#')#") />
		</cfif>
		<cfset TheSize = "o">
		<cfset TheFileName = formatFileName(arguments.type,imageBean.getID(),TheSize,arguments.fileext)>
		<cfset ImageWrite(oImage,imagePath & TheFileName)>
		<!---<cfset ImageWrite(myimage,expandPath('\_uploads\images\_original\#imguuid#.jpg'),1) />
		<cfset oImage = ImageRead(imageData) />
		...
		<cfset returnVar.status = true />
		<cfset returnVar.statusMsg = "Successfully processed the image." />
		<cfset returnVar.imageId = imageBean.getID() />
		<cfset returnVar.caption = imageBean.getCaption() />
		<cfset returnVar.fileName = formatFileName(arguments.type,imageBean.getID(),'t',arguments.fileext) />
		<cfset returnVar.fileExt = arguments.fileext />
