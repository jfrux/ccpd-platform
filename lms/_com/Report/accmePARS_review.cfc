<cfcomponent displayname="ACCME Summary Report" output="no">
	...
	<!--- CONFIGURATION --->
	<cfset ReportPath = ExpandPath("/admin/_reports/28/")>
	<cfset ReportFileName = "accmePARS-review-#DateFormat(Now(),'MMDDYY')##TimeFormat(Now(),'hhmmss')#-#Session.AccountID#.txt">
	<cfset ColumnCount = 3>
	...
	<cffunction name="Run" access="remote" output="yes">
		...
		<cfif Status EQ "">
			<cfset qACCMEData = getACCMEData(argumentCollection=arguments) />
			...
			<cfloop query="qACCMEData">  
				...
				<cfscript>
					...
					querySetCell(qReport,"TotalExpenses",qACCMEData.TotalExpenses);
					...
					<poi:sheets>
						<poi:sheet name="#BookName#" orientation="landscape">
							<poi:columns>
								...
								<poi:column style="width:50px;" />
								<poi:column style="width:70px;" /> <!--- CITY --->
								<poi:column style="width:50px;" /> <!--- STATE --->
								<poi:column style="width:50px;" /> <!--- COUNTRY --->
								<poi:column style="width:50px;" />
								<poi:column style="width:50px;" />
								<poi:column style="width:50px;" />
								<poi:column style="width:50px;" />
							...
							<poi:row class="title">
								...
								<poi:cell value="Type" />
								<poi:cell value="Hours" />
								<poi:cell value="Phys" />
								<poi:cell value="Non-Phys" />
								<poi:cell value="Support Count" />
								<poi:cell value="Total $" />
								<poi:cell value="CoSponsor" />
								<poi:cell value="Credits Desg." />
								<poi:cell value="Advt/Exh.Inc." />
								<poi:cell value="Oth.Inc." />
								<poi:cell value="Exp." />
							...
							<poi:row class="row">
								...
								<!--- EXPENSES --->
								<cfif qReport.TotalExpenses>
									<poi:cell value="#qReport.TotalExpenses#" type="numeric" numberformat="0.00" />
								<cfelse>
									<poi:cell value="" />
								</cfif>
							</poi:row>
...
<cffunction name="getACCMEData" access="public">
	<cfargument name="startDate" type="string" required="yes" />
	<cfargument name="endDate" type="string" required="yes" />
	<cfargument name="requireCredit" type="boolean" required="no" default="true" />
	<cfargument name="requireAttendees" type="boolean" required="no" default="true" />
		...
		*/
		...
		isNull(
			(SELECT 
				sum(ledger.amount)
			FROM 
				ceschema.ce_Activity_FinLedger As ledger
			INNER JOIN 
				ceschema.ce_sys_entrytype As type ON type.entrytypeid=ledger.entrytypeid
			WHERE 
