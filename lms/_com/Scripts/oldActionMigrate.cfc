<cfcomponent displayname="Migrate old actions to new history" output="yes">
	<cfset PathToFile = ExpandPath('/admin/_com/Scripts/ActionMigration.csv')>
...
ORDER BY ShortName
		...
		<cfloop query="qActions">
			...
			<cfquery name="qNewStyle" dbtype="query" timeout="#CreateTimeSpan(1,0,0,0)#">
				...
				WHERE OldTitles LIKE <cfqueryparam value="%#Trim(qActions.ShortName)#%" cfsqltype="cf_sql_varchar" />
			</cfquery>
...
SELECT 
	...
	ToPersonID = PersonID, ToActivityID = ActivityID, FromPersonID = CreatedBy,ToContent = Replace(LongName,'/index.cfm','/admin/index.cfm'),Created
FROM ce_Action
WHERE ShortName IS NOT NULL AND ShortName <> '' AND MovedFlag='N'
ORDER BY ShortName
		</cfquery>
