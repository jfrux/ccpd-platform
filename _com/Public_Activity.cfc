<cfcomponent extends="public">
  ...
  <cffunction name="init" access="public" output="false" returntype="_com.public_activity">
  ...
  <cffunction name="AutoComplete" access="Public" output="no" returntype="string">
    ...
    <cfoutput>
      <cfloop query="qList">
        #qList.Title#
        <cfif qList.RecordCount NEQ qList.CurrentRow>
          #Chr(10)#
        </cfif>
      </cfloop>
    </cfoutput>
  ...
  <cffunction name="getGroupings" hint="Returns JSON data for groupings." access="Public" output="false" returntype="string">
    ...
    <cfif GroupingList.RecordCount GT 0>
      <cfoutput>
        ...
        {
        <cfloop query="GroupingList">
          "#GroupingList.GroupingID#":"#GroupingList.Name#"
          <cfif GroupingList.CurrentRow NEQ GroupingList.RecordCount>
            ,
          </cfif>
        </cfloop>
        }
  ...
  <cffunction name="saveNote" access="Public" output="true" returntype="struct">
    ...
    <cfif arrayLen(status.getErrors()) EQ 0>
      <cfset noteSaved = Application.Com.ActivityNoteDAO.Save(ActivityNoteBean)>
      
      <cfset activityNoteBean.setNoteId(noteSaved) />
      <cfset activityNoteBean = Application.com.activityNoteDAO.read(activityNoteBean) />
      
      <cfset payLoad = {
        id: activityNoteBean.getNoteId(),
        body: activityNoteBean.getBody(),
        author:{
          id: session.personid,
          name: session.person.getCertName()
        },
        timestamp: activityNoteBean.getCreated()
      } />
    ...
    <cfif NOT AttendeeExists>
      ...
      <cfif qPersonDegree.RecordCount GT 0>
        ...
        <cfif TermsNeeded EQ "Y">
          <cfset AttendeeBean.setStatusID("5")>
          <!--- PENDING STATUS --->
