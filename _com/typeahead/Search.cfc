<cfcomponent displayname="" output="no">
  ...
  <cffunction name="all" access="public" returntype="any" output="no">
  ...
  <cffunction name="activity" hint="returns activity list for typeahead" access="public" output="no">
    <cfargument name="q" type="string" required="no" default="" />
    <cfargument name="max" type="numeric" required="no" default="10" />
  ...
  <cffunction name="person" hint="returns person list for typeahead" access="public" returntype="any" output="no">
    <cfargument name="q" type="string" required="no" default="" />
    <cfargument name="max" type="numeric" required="no" default="10" />
    ...
    <cfif len(trim(searchStr)) GT 0>
      <cfquery name="query" datasource="#variables.dsn#">
        ...
        users AS obj
        INNER JOIN
        CONTAINSTABLE(users, (certname), @search) AS fts ON obj.personId = fts.[KEY] 
  ...
  <cffunction name="city" hint="returns folders list for typeahead" access="public" returntype="query" output="no">
    ...
    <cfquery name="query" datasource="#variables.dsn#">
      ...
      SELECT 
        ...
        type = 'city'
      FROM 
      geonameCities AS obj
      INNER JOIN
      CONTAINSTABLE(geonameCities, (name,admin1code), @search) AS fts ON obj.geonameid = fts.[KEY] 
      ORDER BY fts.[RANK] DESC
    </cfquery>
  ...
  <cffunction name="searchFormatter" hint="formats search query string for full-text methods" access="private" output="no" returntype="string">
    ...
    <cfif len(trim(arguments.q)) GT 0>
